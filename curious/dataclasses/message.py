...
class Message(Dataclass):
    ...
    async def clean_content(self) -> str:
        ...
        """
        return await current_bot.get().clean_content(self.content)
    ...
    async def get_invites(self) -> 'typing.List[dt_invite.Invite]':
        ...
        for match in invites:
            ...
            try:
                obbs.append(await current_bot.get().get_invite(code))
    ...
    async def delete(self) -> None:
        ...
        await current_bot.get().http.delete_message(self.channel.id, self.id)
    ...
    async def edit(self, new_content: str = None, *,
        ...
        async with current_bot.get().events.wait_for_manager("message_update",
                                                     lambda o, n: n.id == self.id):
            await current_bot.get().http.edit_message(self.channel.id, self.id, content=new_content,
                                              embed=embed)
        return self
    ...
    async def pin(self) -> 'Message':
        ...
        await current_bot.get().http.pin_message(self.channel.id, self.id)
        return self
    ...
    async def unpin(self) -> 'Message':
        ...
        await current_bot.get().http.unpin_message(self.channel.id, self.id)
        return self
    ...
    async def get_who_reacted(self, emoji: 'typing.Union[dt_emoji.Emoji, str]') \
        ...
        reactions = await current_bot.get().http.get_reaction_users(self.channel.id, self.id, emoji)
        result = []

        for user in reactions:
            member_id = int(user.get("id"))
            if self.guild is None:
                result.append(dt_user.User(current_bot.get(), **user))
            else:
    ...
    async def react(self, emoji: 'typing.Union[dt_emoji.Emoji, str]'):
        ...
        await current_bot.get().http.add_reaction(self.channel.id, self.id, emoji)
    ...
    async def unreact(self, reaction: 'typing.Union[dt_emoji.Emoji, str]',
        ...
        await current_bot.get().http.delete_reaction(self.channel.id, self.id, emoji,
    ...
    async def remove_all_reactions(self) -> None:
        ...
        await current_bot.get().http.delete_all_reactions(self.channel.id, self.id)
